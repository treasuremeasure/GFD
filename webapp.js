document.addEventListener("DOMContentLoaded", function () {
    ...
    if (orderFinalButton) {
        ...
        orderFinalButton.replaceWith(orderFinalButton.cloneNode(true));
        const newOrderFinalButton = document.querySelector('.order-summary-button-final');

        let isSubmitting = false; // Flag to prevent double submission

        newOrderFinalButton.addEventListener('click', function () {
            if (!isSubmitting) {
                isSubmitting = true; // Set the flag to true
                sendOrderToAdmin().then(() => {
                    isSubmitting = false; // Reset flag after successful submission
                }).catch(() => {
                    isSubmitting = false; // Reset flag if an error occurs
                });
            }
        });
    }
